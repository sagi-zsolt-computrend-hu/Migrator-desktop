...
public class ExportStep implements Converters, Runnable {
	...
	public ExportStep( AbstractDataSource dataSource ) {
		super();
	...
	public void run() {
		try (var connection = dataSource.getConnection()) {
			var url = Paths.get( connection.getMetaData().getURL()
				.replace( "jdbc:derby:directory://", "" ) ).getParent();
			url = Paths.get( url.toString(), "export1" );
			//			var selectModell = new SelectModell( dataSource, "select * from " /*+ fileName*/ ) // where table_name like ?
			//				.select( /*params*/ );
			log.info( "Export started {}", url );
			Thread.sleep( 1500 );
			...
			log.info( "Export Ended {} ", tableName/*, dataSource.getCount( tableName )*/ );
		}
	}
